  ...
  return (
    <aside className="sidebar w-2/12 h-full overflow-hidden text-[var(--color-sidebar-link)]">
      ...
      {sect ? (sectionList.map(s => (
          <>
            {s == sectionList[0] ? (
              <li onClick={()=>{setSect(s)}} key={s} id="sectList" className="relative rounded-sm p-3 mx-1 flex items-center justify-between hover:font-bold hover:bg-[var(--color-primary)]/10 hover:cursor-pointer transition">    
